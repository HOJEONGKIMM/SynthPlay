...
public class MainActivity extends AppCompatActivity {
    ...
    private SoundPool soundPool; // 사운드풀 객체
    private int soundC, soundCSharp, soundD, soundDSharp, soundE, soundF, soundFSharp, soundG, soundGSharp, soundA, soundASharp, soundB, soundCHigh;
    private boolean soundsLoaded = false; // 사운드 로드 상태 확인

    private static final int REQUEST_RECORD_AUDIO_PERMISSION = 200; // 녹음 권한 요청 코드
    private boolean permissionToRecordAccepted = false; // 녹음 권한 상태
    private boolean isRecording = false; // 녹음 중인지 확인

    private MediaRecorder mediaRecorder; // 녹음기 객체
    private MediaPlayer mediaPlayer; // 미디어 플레이어 객체
    private String recordedFilePath; // 녹음 파일 경로
    ...
    // 캔버스 지우기
    private void clearCanvas() {
        // TODO: 오선지 캔버스 초기화 코드 추가
        Toast.makeText(this, "캔버스가 초기화되었습니다.", Toast.LENGTH_SHORT).show();
    }
    ...
    private void startRecording() {
        if (mediaRecorder == null && permissionToRecordAccepted) {
            mediaRecorder = new MediaRecorder();
            mediaRecorder.setAudioSource(MediaRecorder.AudioSource.UNPROCESSED); // 원음 녹음
            mediaRecorder.setOutputFormat(MediaRecorder.OutputFormat.MPEG_4);
            mediaRecorder.setAudioEncoder(MediaRecorder.AudioEncoder.AAC);
            mediaRecorder.setAudioSamplingRate(44100); // CD 품질
            mediaRecorder.setAudioEncodingBitRate(96000); // 고품질 비트레이트

            recordedFilePath = getFilesDir().getAbsolutePath() + "/recorded_audio_" + System.currentTimeMillis() + ".mp4";
            mediaRecorder.setOutputFile(recordedFilePath);
